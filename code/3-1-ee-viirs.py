...
Calculates annual nightlights values by historical ethnic region from 2014-2018 using VIIRS data from the Google Earth Engine
"""
...
shapefile = ee.FeatureCollection('users/gradykilleen/slave-trade') # Upload shapefile containing boundary data  of the polygons of interest (e.g. using the Google Earth Engine console) and insert the asset ID here, in single or double quotes
...
output_file = 'viirs' # Output file name  
...
# Rename column from sum to viirs 
def rename(feature):
    return ee.Feature(None, {'old_ethnic': feature.get('old_ethnic'), 'Date': feature.get('Date'), 'viirs': feature.get('sum')})

to_export = zonal_stats_no_geometry.map(rename)

# Export the data to Google Drive
task = ee.batch.Export.table.toDrive(collection=to_export, description=output_file, 
                        fileFormat='CSV', fileNamePrefix=output_file,
                        folder=output_folder,
                        selectors=['old_ethnic', 'Date', 'viirs'])
